...
2. 使用场景:
   - [业务上全局应该只有一份]某个类只要一个对象, 或创建一个类需要消耗的资源过多
   - 如 配置类, 访问 IO 和 数据库操作, 线程池, 数据库连接池, ID 生成器类[业务上全局应该只有一份]
   - _可以使用单例解决资源访问冲突的问题: log 问题_
   - **如果单例类并没有后续扩展的需求, 并且不依赖外部系统**
3. UML 图
...
4. feature
   ...
   - [创建]构造函数访问修饰符设置为 private
   - [创建]两种破坏: 反序列化, 反射
   - [访问{性能}]通过一个静态方法或者枚举返回单例类对象
   - [线程]多线程环境下的线程安全问题
...
6. 单例存在哪些问题
   ...
   - 对 OOP 的支持不好: 对 抽象、继承、多态 都不友好
     1. 违反基于接口而非实现的设计原则
     2. 比如 Id 生成器, 改成不同业务使用不同的策略, 就需要改所有的代码
   - 单例会隐藏类之间的依赖关系: 不需要创建, 阅读差
   - 单例对代码的扩展性不友好
     1. 比如数据库连接池的隔离, 现在有些就不是单例的
   - 单例对代码的可测试性不友好
   - _单例不支持有参数的构造函数_

7. 单例与静态类的区别？
8. 有何替代的解决方案
   - 全局唯一也可以通过 工厂模式、IOC 容器 来保证

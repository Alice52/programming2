[toc]

### 简介

**软件设计的发展:**

1. 功能分解法 -- 计算任务, 以 `功能` 为中心
2. 结构化程序设计 -- 以 `数据` 为中心
3. 面向对象的程序设计 -- 以 `对象` 为中心
4. 组件化程序设计 -- 以 `组件` 为中心

---

### 模板编程

1. **公用模块 = 原始需求 + 扩展需求 + 封装 + 接口**
2. 无论是从业务角度还是从架构、技术角度, 模块化首先意味着将 `代码、数据` 等内容按照其 `职责不同分离`, 使其变得更加容易维护、迭代, 使开发人员可以分而治之。
3. 按照项目业务划分的大模块, `模块化侧重于数据的封装`, _一组相关的组件定义成一个模块_, 一个 JSON 对象可以是一个模块。

### 组件化编程

1. 思想: 分而治之
2. **定义: 可独立发布的二进制单元, 单独开发, 编译和单独测试**

   - 黑盒, 具有版本号, 配置并调用接口使用。
   - `接口-实现分离`, 组件间通过接口联系
   - 自行管理内部的一个或多个类(模块化管理)

3. **front:**

   - 组件是具体的: 按照一些小功能的 `通用性和可复用性` 来抽象组件组件化更多的 `关注 UI 部分`, 比如用户看到的弹出框, 页脚, 确认按钮等, 这些组件可以组合成新的组件, 又可以和其他组件组合组合成新的组件
   - 特性:
     1. 组件是对逻辑的封装, 不限于图形元素
     2. 组件具备单个可移植性
     3. 组件是[声明式定义](#声明式编程)的, 而非命令式

---

### 声明式编程

1. 是什么

   - 它描述目标的性质, 让计算机明白目标, 而非流程
   - 命令式编程则需要用算法来明确的指出每一步该怎么做
   - 声明式编程不用告诉计算机问题领域, 从而避免随之而来的副作用

2. 优点

   - 代码的可读性变高
   - 减小代码量, 从而减小低级错误出现的概率
   - 代码安全: 因为内部实现的细节与其中的数据不能被修改{immutable}
   - 代码的复用性高
   - 代码的耦合度低
   - 代码的使用与集成简单: 根据别人的语法做一些声明就可以有一定的效果 & 降低接入成本
   - 代码的灵活性高: **将能力抽象化**, 使其能够通过配置的方式灵活的应用于需要的场景
   - 可以做到屏蔽了理解层面、实施层面上的差异化: `@DateTimeFormat`

3. 缺点

   - 熟悉将能力抽象化及相关规范, 这个很难
   - 代码的定制化成都低: 因为是封装好的
   - 声明式下出现错误比较难处理

4. _声明式编程下的不同风格: `logic, functional, data-driven`_

   - logic: 通过预先存在的正确的逻辑, 对输入进行结果, 多用于数学{和函数类似}
   - functional: 函数
   - data-driven: sql

5. 常见的声明式语言

   - 前端: HTML, SQL, CSS and XML
   - 后端: _Prolog, Haskell, Miranda, XQuery and Lisp_

6. 应用

   - java 中通过注解做声明是比较优雅的: 比如定义限流, 延迟, 幂等等声明
   - 定义特定功能的函数, 也算是某种声明

### 命令式编程

1. 定义

   - **每一个步骤都是要自己去定义实现的.**

2. 优点

   - 简单易学, 经验丰富{java}
   - 程序的可定制化成都比较高: 自己写的想怎么改就怎么改{循环 | 状态修改等}

3. 常见的命令式语言

   - **java**: 可以自己写声明式的代码
   - c | c++
   - php

4. 适应场景

   - 每一步都需要精确控制
   - 项目的工作量不大等不常变化

---

### 声明式 vs 命令式

1. 核心{focus on}

   - 声明式: 描述最终的目的
   - 命令式: 描述每一步的执行过程

2. 定制化代码

   - 声明式: 几乎没啥定制
   - 命令式: all in control

3. 代码量

   - 声明式: 非常少
   - 命令式: 很多

4. sample

   ```php
   // 获取 value 值
   $participantlist = [1 => 'Grey', 2 => 'Devon', 3 => 'Kor'];
   ```

   - 声明式
     ```php
     $lastnames = array_values ($participantlist);
     ```
   - 函数式
     ```php
     $lastnames = [];
     Foreach ($participantlist as $id => $name) {
                    $lastnames[] = $name;
     }
     ```

---

### 面向过程编程

1. 定义
   - 是什么: 是一种**流程化**编程范式或编程风格
   - 以问题的解决步骤为中心, 将业务逻辑封装成一个个可以顺序执行的方法, 通过组织一系列方法的顺序执行实现功能, **仅支持面向过程编程**
   - 基本单元: 以过程[方法、函数、操作]作为组织代码的基本单元, 数据与方法分离
   - 特性: 数据[成员变量、属性]与方法相分离为最主要的特点 + 不支持类&对象的概念[四大特性]
2. 具体代码体现
   - 面向过程风格的代码被组织成了一组方法集合及其数据结构[struct User], 方法和数据结构的定义是分开的
   - 开发初期需要将错综复杂的流程翻译成顺序执行的方法
3. 自顶向下
   - 规划程序: 整体上理解问题
   - 分解任务: 将问题拆分成多个子任务, 每个子任务都可使用函数表示
   - 设计过程: 每个过程的设计关注输入输出和实现细节
   - 组合过程: 组织顺序调用

---

### [函数式编程](https://www.jianshu.com/p/d59fa2ef0d78)

1. 函数: 数学含义的函数, 而不是方法, 简单的说就是从 A(定义域)到 B(值域)的一个映射过程
2. 定义: 将业务逻辑细化、抽象、封装成一个个 **`无状态的`** `功能函数`，并借助语言自带的高阶函数 `API`, 将整个业务流程转化为函数之间的相互调用.
3. 思想: 面向过程的程序设计, 但其思想更接近数学计算
4. 作用: 计算, 在科学计算、数据处理、统计分析等容易用数学表达式来表示的领域可以高效计算
5. 优点:

   - 函数的执行结果只与入参有关: 跟其他任何外部变量无关
   - 同样的入参执行结果都一样
   - ~~函数式编程中 `没有赋值语句`, 因此变量一旦有了值就不会再改变.~~
   - ~~顺序无关性~~
   - ~~引用透明: 因为没有能够改变表达式值的副作用, 可以在任何时候对它求值. 由于能够在任何时候对表达式求职, 所以可以用变量的值来自由替换表达式.~~
   - ~~函数式程序更加模块化.~~
   - ~~函数式程序总是线程安全的, 因为防止了共享状态的变化.~~

6. 与面向过程编程的区别

   - 区别在于它的**编程思想**: 函数式编程认为程序可以用一系列数学函数或表达式的组合来表示, 函数式编程是程序面向数学的更底层的抽象, 将计算过程描述为表达式
   - 函数式下的方法函数是**无状态**的: 无 oop 的类共享成员变量, 也没有过程编程的共享全局变量

     ```java
     // 有状态函数
     int b;
     int increase(int a) {
        return a + b;
     }

     // 无状态函数
     int increase(int a, int b) {
        return a + b;
     }
     ```

7. java8 的函数式相关

   - lambda
   - stream
   - funcational

---

### [面向对象编程](https://www.cnblogs.com/zhenw/p/6160931.html)

1. 定义: 将业务逻辑细化、抽象、封装成一个个对象, 并借助语言、库、组件、框架等, 将整个业务流程转化为对象之间的相互调用, 这就是面向对象编程.
2. 个人定义
   - 是什么: 是一种编程范式或编程风格
   - 基本单元: 只要某种编程语言**支持类或对象的语法概念**, 并且以此作为组织代码的**基本单元**
   - 特性: 以类、对象作为组织代码的单元以及四大特性{封装, 抽象, 继承, 多态}
   - 面向对象编程语言是支持类或对象的语法机制，并有现成的语法机制, 可以简单的实现四大特性
3. 具体的代码体现
   - 面向对象风格的代码被组织成一组类, 方法和数据结构被绑定一起, 定义在类中
   - 开发初期需要先将需求翻译成类, 然后按照处理流程组装在一起
4. 面向对象编程语言: 现有语法能方便的实现 OOP 的四大特性的语言

---

### 面向过程编程 & 面向函数编程 & 面向对象编程 区别

1. 过程: 过程式 --> 函数式 --> **面向对象**
2. 面向对象编程相比面向过程编程有哪些优势
   - OOP 更加能够应对大规模复杂程序的开发: 过程编程流程很难梳理
   - OOP 封装[可以都有]、抽象[都有]、继承[oop]、多态[oop]: 易扩展、易复用、易维护
3. 看似是面向对象, 实际是面向过程的代码
   - 滥用 getter、setter 方法: 封装中的数据保护性和一致性{购物车}消失[Collections.unmodifiableList()/clone()]
   - 滥用全局变量和全局方法: 静态 util 方法无需创建对象就可使用, 数据与方法分离, 典型的面向过程风格
   - 定义数据和方法分离的类: 比如 mvc 架构, 但是广义上看 controller 有属性与方法, 也是一个对象，所以一定程度上也是面向对象的
4. 为什么容易写出面向过程风格的代码
   - 面向过程编程风格符合人的这种流程化思维方式
   - 面向对象编程风格正好相反它是一种**自底向上**的思考方式: `types/oop.md#6` (模块化+可管理)
   - 面向对象编程要比面向过程编程难一些: 类的设计是很难的
5. 最终的目的还是写出易维护、易读、易复用、易扩展的高质量代码
6. 现状
   - 纯面向过程的编程方法: 很少, 其多服务与底层, 比如 c
   - 函数式编程: 用在科学计算、大数据处理等特殊领域局限的场景, **所以大多作为面向对象编程的一种补充**

---

### reference

1. [组件化和模块化](https://www.jianshu.com/p/a7df83270a8b)
2. [函数式编程](https://www.jianshu.com/p/d59fa2ef0d78)
3. [面向对象编程](https://www.cnblogs.com/zhenw/p/6160931.html)
4. [函数式&声明式](https://www.techtarget.com/searchapparchitecture/tip/The-basics-of-working-with-declarative-programming-languages)
